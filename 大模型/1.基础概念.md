...
# 四、工具介绍
## 1、Cursor
基于 VS Code 魔改的AI 原生编辑器，AI 深度内置，不是插件，集成 GPT-4、Claude 3.5 等大模型。
想用自己的 API Key → 必须开 Cursor Pro；免费版只能用 Cursor 自带模型，用自己的key没法tab补全。

***与VS Code + Copilot 区别***
* 集成深度：Cursor 底层集成；Copilot 是插件，上下文有限
* 多文件编辑：Cursor 原生支持；Copilot 需手动切换文件
* 自然语言交互：Cursor 全流程支持；Copilot 仅补全，聊天需切页面
* 上手难度：Cursor 更低，适合新手；Copilot 需熟悉插件生态

## 2、Github Copilot
GitHub 与 OpenAI 联合推出的 AI 编程助手，简单说就是：能在你写代码时实时给建议、补全代码、生成函数 / 测试 / 注释的 “智能结对程序员”。定位：IDE 插件 + 云端大模型（GPT-4o / Codex 等）。

>在 VS Code 等编辑器里敲代码 → 插件把上下文（当前文件、附近代码、注释）发给 GitHub 云端;云端用大语言模型（LLM） 分析，返回最可能的代码片段 → 灰色提示显示，按 Tab 即可接受;现在支持多模型切换：GPT-4o、Claude、Gemini 等

## 3、CC Switch
CC Switch 是一款跨平台桌面工具，用来统一管理多款 AI 编程命令行（CLI）工具与 API 供应商配置，实现一键切换与集中管理。

>Claude Code → CC Switch，早期主打管理 Claude Code，后来扩展到多款 AI 编程工具。

**适用于**：Claude Code、OpenAI Codex CLI、Google Gemini CLI、OpenCode、OpenClaw
**主要解决痛点**：各工具配置格式不统一（JSON/TOML/.env）、切换 API 供应商要手动改配置文件、MCP、Skills、提示词没法统一管理、配置易丢失、损坏

## 4、Claude Code
Claude Code 是 Anthropic 推出的「终端原生、能自主干活的 AI 编程智能体（Agent）」—— 简单说，它是一个跑在你终端里的 “AI 程序员”，能直接读代码、改文件、跑命令、修 Bug、管理 Git，而不只是补几行代码。

>claude有自己的大模型，但是订阅和买api key都需要注册账号，国内不让用，且容易被封号。

**推荐使用方式：Claude Code + CC switch + deepseek_api**
**配合cursor使用更佳，可以同时享受 Cursor 本身的自动补全，和 Claude Code 强大的复杂任务处理能力。**
...
## 5、Codex
全称：OpenAI Codex Agent；定位：云端 + 本地桌面的 “AI 软件工程师”；和Claude Code的功能差不多。
>codex使用最好订阅chatgpt或者买gpt的api key，比较稳定，不容易被封号。Codex默认使用 OpenAI 的 Responses API，而 DeepSeek 官方目前主要支持的是 Chat/Completions API。无法直连，搭本地代理做协议转换。DeepSeek 只兼容 OpenAI 的 “Chat Completions API”，但新版 Codex 强制用 OpenAI 私有的 “Responses API”，俩协议不互通
