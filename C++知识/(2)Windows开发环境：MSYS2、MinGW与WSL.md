...
1. 在 Windows 上安装 WSL（推荐 WSL2 + Ubuntu）
2. VS Code 安装 **WSL 插件**，远程连接 WSL
3. 在 WSL 中安装开发工具：`sudo apt install gcc g++ cmake`
4. 代理配置：Clash 开启 TUN 模式，WSL 流量自动经过 Windows 代理
5. 配置 Git：`git config --global user.name / user.email`

> WSL 里执行 `git pull`、`git push` 时，流量走 Windows Clash 代理再到 GitHub。
